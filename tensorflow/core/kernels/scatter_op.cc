 ...
 private:
  ...
  void DoCompute(OpKernelContext* c) {
    ...
    if (N > 0) {
      ...
      Index* data_host =
          static_cast<Index*>(tensorflow::cpu_allocator()->AllocateRaw(
              EIGEN_MAX_ALIGN_BYTES, index_size));
      auto src_ptr = GetBase(&indices);
      c->eigen_sycl_device().memcpyDeviceToHost(
          data_host, static_cast<const Index*>(src_ptr), index_size);
      ...
      const Index bad_i =
          functor(c, c->template eigen_device<SYCLDevice>(), params_flat,
                  updates_flat, data_host, indices.NumElements());
      OP_REQUIRES(
          c, bad_i < 0,
          errors::InvalidArgument(
              "indices", SliceDebugString(indices.shape(), bad_i), " = ",
